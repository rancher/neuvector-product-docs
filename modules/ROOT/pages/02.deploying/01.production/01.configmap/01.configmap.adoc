...
Note that the secret is referred to in the standard Kubernetes and OpenShift Controller link:/deploying/kubernetes#deploy-neuvector[deployment yaml files] under Volumes.

=== ConfigMaps and Persistent Storage

Both the ConfigMaps and the link:/deploying/production#backups-and-persistent-data[persistent storage] backup are only read when a new NeuVector cluster is deployed, or the cluster fails and is restarted. They are not used during rolling upgrades.
