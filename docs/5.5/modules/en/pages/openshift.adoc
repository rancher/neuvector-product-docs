...
[#_deploy-separate-product-components-with-redhat-openshift]
...
The sample file below will deploy one manager, 3 controllers, and 2 scanner pods. It will deploy an enforcer on every node as a daemonset, including on the master node (if schedulable). See the bottom section for specifying dedicated manager or controller nodes using node labels. Note: It is not recommended to deploy (scale) more than one manager behind a load balancer due to potential session state issues. If you plan to use a PersistentVolume claim to store the backup of {product-name} config files, please see the general Backup/Persistent Data section in the xref:production.adoc#_backups-and-persistent-data[Production Deployment] overview.
...
[#_updating-the-cve-database-on-openshift-deployments]
...
[#_enable-disable-scheduling-on-the-master-node]
...
[#_openshift-deployment-in-non-privileged-mode]
