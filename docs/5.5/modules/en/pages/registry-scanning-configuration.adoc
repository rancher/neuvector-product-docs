...
:description: Configure registry scanning using SUSE® Security containers to manage registries and repositories and scan images for vulnerabilities
:revdate: 2025-06-25
...
To configure registries and repositories to be scanning, go to the Assets _> Registries menu in the {product-name} console. Add or edit registries to be scanned. Use the Filter to define repositories or subsets of images to be scanned. If your registry requires access through a proxy, this can be configured in Settings _> Configuration.
...
[#_configuring_proxy_server_for_registry]
...
[#_native_docker_registry_also_quay_and_harbor]
=== Native Docker registry (also Quay and Harbor)

[#_add_native_docker_registry]
...
[#_start_scanning_the_docker_registry]
...
[#_add_openshift_registry_with_username_and_password]
...
[#_add_openshift_registry_with_token]
...
[#_stability_issues_in_openshift_37_registry]
...
[#_add_jfrog_artifactory_registry_docker_access_method_repository_path]
...
[#_adding_jfrog_artifactory_registry_docker_access_method_subdomain]
...
[#_add_jfrog_artifactory_registry_docker_access_method_port]
...
[#_add_saas_jfrog_artifactory_registry_docker_access_method_port]
...
[#_start_scanning_a_jfrog_artifactory_registry]
...
[#_neuvector_registry_scanning_with_jfrog_registry]
==== NeuVector registry scanning with JFrog registry


For NeuVector registry scanning with a JFrog registry in subdomain mode, the subdomain is typically the prefix of the JFrog registry URL.

**Standard JFrog Subdomain Example**
...
Registry filter example: `mysubdomain/docker-service-broker-tmp-local/devop/test/joe/*`

**When JFrog Uses a Tweaked Subdomain**

When you use a modified subdomain in the registry URL.

JFrog registry URL: `\https://artifact-mysubdomain.myhost.com`

* **artifact-mysubdomain**: modified subdomain used only in the registry URL
* **mysubdomain**: actual subdomain in the JFrog server
* **myhost.com**: JFrog registry hostname

Registry filter example: `myrepo/docker-service-broker-tmp-local/devop/test/joe/*`

[NOTE]
====
NeuVector previously supported these environments. However, support broke after upgrading to newer JFrog server versions because NeuVector cannot determine whether the true subdomain value comes from the URL or the filter (for example, artifact-mysubdomain vs. mysubdomain).
====

**Required Fix**

If subdomain is not the prefix of the JFrog registry server URL, you must configure the registry filter using this format:
...
[NOTE]
====
The required format: `<actual-subdomain>/...`
====

By specifying the filter in this format, NeuVector can correctly identify the actual subdomain used for communication with the JFrog server in subdomain mode.

[#_google_container_registry]
...
[#_enable_cloud_resource_manager_api_for_the_project]
...
[#_create_key_for_container_service_account]
...
[#_copy_json_file_to_client_machine]
==== Copy json file to client machine

[#_add_google_container_registry_from_the_product_gui]
...
[#_start_scanning_a_google_container_registry]
...
[#_azure_container_registry]
=== Azure Container Registry

Ref:
https://azure.microsoft.com/en-us/services/container-registry/

[#_obtain_azure_container_username_and_password_as_shown_below]
...
[#_add_azure_container_registry_from_the_product_gui]
...
[#_start_scanning_a_azure_container_registry]
...
[#_sonatype_nexus_docker_registry]
...
[#_add_sonatype_nexus_docker_registry]
...
[#_start_scanning_sonatype_nexus_docker_registry]
...
[#_sample_gitlab_environmnent_configurations]
...
[#_obtain_gitlab_private_token_as_shown_below]
...
[#_obtaining_external_and_registry_urls]
...
[#_add_gitlab_registry_from_the_product_console]
...
[#_ibm_cloud_container_registry]
=== IBM Cloud Container Registry

Ref:  https://www.ibm.com/cloud/container-registry

[#_add_ibm_container_registry]
...
[#_add_the_github_container_registry]
