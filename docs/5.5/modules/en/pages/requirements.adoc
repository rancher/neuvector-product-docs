...
* For configuration backup/HA, a RWX PVC of 1Gi or more. See xref:production.adoc#_backups-and-persistent-data[Backups and Persistent Data section] for more details.
...
For additional advice on performance and sizing, see the xref:production.adoc#_best-practices-tips-qa-for-deploying-and-managing-product[Onboarding/Best Practices section].
...
[#_adding-scaling-constraints-for-large-workload-environments]
