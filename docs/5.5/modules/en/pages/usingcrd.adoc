...
[#_product-crd-for-policy-as-code]
...
The `NvGroupDefinition` custom resource represents the definition of a group, including its description and criteria. It does not represent an active or enforced group on its own. Instead, it serves as a reference definition within the NeuVector system.

NeuVector creates and applies active groups only when an `NvGroupDefinition` is referenced in an `NvSecurityRule` or `NvClusterSecurityRule`. Until then, the `NvGroupDefinition` exists in the Kubernetes cluster without any enforcement or runtime effect. This behavior is intentional and part of NeuVector’s design.
...
[#_schema-attribute-name-referral]
...
[#_nvclustersecurityrule-and-nvsecurityrule]
...
[#_enabling-crd-support]
==== Enabling CRD Support

As described in the xref:kubernetes.adoc#_deploy-using-kubernetes[Kubernetes] and xref:openshift.adoc#_deploy-on-openshift[OpenShift] deployment sections (Deploying {product-name}),  the appropriate clusterroles and clusterrole bindings for custom resources and NvSecurityRules should be added first.
...
[#_generating-a-sample-product-crd]
...
[#_sample-neuvector-crd-nvadmissioncontrolsecurityrule]
...
[#_policy-mode-configuration-and-group-definition]
...
[#_policy-mode-configuration-requirements]
...
[#_crd-policy-rules-syntax-and-semantics]
...
[#_rolebinding-clusterolebinding-with-2-users-in-different-namespaces-to-a-clusterrole-nvsecurityrules-nvclustersecurityrules-resources]
...
[#_example-using-2-different-types-of-defined-yaml-files-and-the-effect-of-using-each-user]
...
[#_expressing-network-rules-ingress-egress-objects-in-crds]
...
[#_customized-configurations-for-deployed-applications]
...
[#_examples-of-crd-configurations-that-perform-these-functions]
...
[#_policy-groups-and-rules-migration-support]
...
[#_example-of-an-exported-group-yaml-file-with-the-nvsecurityrule-resource-type]
...
[#_example-of-an-exported-group-yaml-file-with-the-nvclustersecurityrule-resource-type]
...
[#_unsupported-export-group-types]
...
[#_updating-crd-rules-and-adding-to-existing-groups]
...
[#_dynamic-criteria-support-for-nvclustersecurityrule]
