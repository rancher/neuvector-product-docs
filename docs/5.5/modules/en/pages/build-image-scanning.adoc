...
[#_ci-cd-build-phase-vulnerability-scanning]
...
For Kubernetes or OpenShift-based local scanning, remove the commented-out section of the sample scanner deployment yaml file, shown in the xref:kubernetes.adoc#_deploy-using-kubernetes[Deploying {product-name}] sections. The commented out section looks like this:
...
For Docker-native local scanning, follow the instructions for Docker scanner deployments in the xref:docker.adoc#_deploy-the-product-scanner-container[Docker Production deployments] section for the scanner.

[#_local-build-phase-scanning-scanner-only-no-controller-required]
