...
. Le dépôt doit être le nom complet si l'organisation est nulle pour le registre docker public ou ajouter 'library' avant le dépôt comme indiqué ci_dessus.
. Créez un dépôt virtuel et ajoutez tous les dépôts locaux pour numériser tous les tags sur un registre JFrog avec la méthode d'accès au sous_domaine docker.
