...
== GitLabビルドパイプライン中の脆弱性をスキャンする

{product-name}は、GitLabビルドパイプラインでトリガーされた脆弱性をスキャンするように設定できます。ここに設定して使用可能な https://gitlab.com/neuvector/gitlab-plugin[GitLabプラグイン]があります。プラグインの使用については、GitLabサイトの指示に従ってください。

スキャンは、下記のスクリプトを設定してコントローラーにアクセスすることにより、{product-name} REST APIも使用できます。

さらに、Allinoneまたはコントローラーに接続するようにデプロイされ、設定された{product-name}スキャナコンテナがあることを確認してください。4.0以降では、neuvector/scannerコンテナはallinoneまたはコントローラーとは別にデプロイする必要があります。

=== REST APIを使用してGitLabビルド中にスキャンする

脆弱性スキャンをトリガーするために、あなたの{product-name}ログイン資格情報に設定された以下のスクリプトを使用してください。
