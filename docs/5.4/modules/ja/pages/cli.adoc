...
== {product-name}コマンドラインの使い方

{product-name}ソリューションは、CLIを使って限られた機能のみを利用できます。CLIはマネージャー経由で利用でき、マネージャーはREST APIを使ってコントローラーにコマンドを発行します。その後、コントローラーがエンフォーサーを適切に管理します。REST APIを通じて完全な操作セットがサポートされており、コントローラーから直接公開することができます。マネージャーまたはオールインワン用のCLIコマンドを入力することで、{product-name} CLIにアクセスできます。例：
...
ここで、'`allinone`'はコントローラーのコンテナ名です。名前としてコンテナIDを使用する必要がある場合があります。

CLIはマネージャー経由で利用できますが、クエリや自動化にはコントローラーに直接xref:automation.adoc[REST API]を使用することを推奨します。

=== CLIコマンドの例

以下は、最も一般的なCLIコマンドの例です。
...
コンソールで使用するのと同じユーザー/パスワードを使用してください。
...
[NOTE]
====
スニファーファイルは、Enforcerのコンテナ内の/var/neuvector/pcapディレクトリに保存されます。ファイルにアクセスできるよう、ボリュームをゲストマシンのディレクトリまたはローカルシステムのディレクトリにマッピングしてください。例えば、docker-composeファイルのvolumesに'`- /var/neuvector:/var/neuvector`'を追加します。
====

ポッドでパケットキャプチャを開始するには、ID_OR_NAMEフィールドに渡すためのコンテナIDを知っておく必要があります。これを`show container -c <container_name>`で行い、その後`request sniffer start <container_id>`でスニファーを開始します。などです。別の例をあげれば、
...
[IMPORTANT]
====
期間が設定されていない場合、スニファーを停止するためにスニファーIDを見つける必要があります。 これを行うには、`show sniffer -c <containerID>`。 次に、`request sniffer stop <sniffer_ID>`。
====
