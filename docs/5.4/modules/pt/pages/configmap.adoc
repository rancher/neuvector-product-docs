= Implantar Usando ConfigMap
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2025-01-10
:page-revdate: {revdate}
:page-opendocs-origin: /02.deploying/01.production/01.configmap/01.configmap.md
:page-opendocs-slug: /implantar/produção/configmap

[#_kubernetes_configmap]
== Kubernetes ConfigMap

{product-name} suporta configuração automatizada usando o recurso ConfigMap do Kubernetes. Isso permite a implantação de {product-name} contêineres com as configurações, integrações e outras definições apropriadas de forma automatizada.

A configuração 'always_reload: true' pode ser adicionada em qualquer yaml de ConfigMap para forçar o recarregamento desse yaml toda vez que o controlador iniciar (versão 4.3.2+). Caso contrário, o ConfigMap será carregado apenas na inicialização inicial ou após a reinicialização completa do cluster (veja a seção de armazenamento persistente abaixo).

[#_complete_sample_product_configmap_initcfgyaml]
=== Exemplo Completo de {product-name} ConfigMap (initcfg.yaml)

O ConfigMap mais recente pode ser encontrado no seguinte https://raw.githubusercontent.com/neuvector/manifests/main/kubernetes/5.4.0/initcfg.yaml[initcfg.yaml].

O exemplo também é mostrado abaixo. Isso contém todas as configurações disponíveis. Por favor, remova as seções desnecessárias e edite as seções necessárias. 

[NOTE]
====
Se estiver usando ConfigMap em um segredo, veja a seção abaixo para alterações de formatação.
====
...
=== Exemplos de ConfigMap Federado (fedinitcfg.yaml)

NeuVector v5.4.0 suporta automação de Federação através do ConfigMap. Abaixo estão exemplos de configurações `fedinitcfg.yaml` que podem ser aplicadas ao seu cluster primário e clusters gerenciados, dependendo do seu caso de uso.

Exemplo `fedinitcfg.yaml` para o cluster primário:
...
Exemplo `fedinitcfg.yaml` para clusters gerenciados:
...
== Proteja Dados Sensíveis Usando um Segredo

Se dados sensíveis forem incluídos em algumas seções do ConfigMap, um segredo pode ser criado para essas seções com dados sensíveis.

Por exemplo, crie o ConfigMap para seções NÃO sensíveis, como passwordProfile e role:
...
Em seguida, crie um segredo para seções com dados sensíveis, como:
...
[IMPORTANT]
====
Remova o caractere pipe '|' em cada seção, conforme mostrado abaixo.
====

Observe a remoção do caractere pipe abaixo se estiver usando seções de configmap em um segredo, com enabled definido como true, e descomente a seção a ser incluída no segredo.
...
Após o controlador ser implantado, todos os arquivos de configuração do ConfigMap e do segredo serão armazenados na pasta /etc/config.

Observe que o segredo é referenciado nos arquivos yaml de implantação padrão do Kubernetes e do OpenShift xref:kubernetes.adoc#_deploy_suse_security[arquivos yaml de implantação] sob Volumes.

[#_configmaps_and_persistent_storage]
== ConfigMaps e Armazenamento Persistente

Tanto os ConfigMaps quanto o xref:production.adoc#_backups_and_persistent_data[backup de armazenamento persistente] são lidos somente quando um novo {product-name} cluster é implantado ou quando o cluster falha e é reiniciado. Eles não são usados durante atualizações gradativas.

O backup de configuração de armazenamento persistente é lido primeiro, depois os ConfigMaps são aplicados, portanto, as configurações do ConfigMap têm precedência. Todas as configurações do ConfigMap (por exemplo, atualizações) também serão salvas no armazenamento persistente.
