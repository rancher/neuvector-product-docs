...
* **NVSHAS-10186**:成功导出到远程储存库后返回 `file_path`。
...
* **NVSHAS-10180**:容器扫描未能检测到与 `package.json` 相关的模块。
...
* **NVSHAS-10156**:NeuVector Scanner 未报告 npm 储存库 `tinymce` 的 CVE。
...
* https://github.com/neuvector/neuvector/security/advisories/GHSA-4jj9-cgqc-x9h5[OpenID Connect易受中间人攻击（MITM）]
...
* *NVSHAS-10084*:将与入场控制相关的CRD结构`nvsecurityrule_def.go`从`resource/`移动到`api/`，以解决构建`neuvector-kubewarden-policy-converter`时的依赖问题。
...
* *NVSHAS-10054*:更新逻辑以避免重复重置 Kubernetes `ValidatingWebhookConfiguration` 并防止无限更新循环。
...
* https://github.com/neuvector/neuvector/security/advisories/GHSA-h773-7gf7-9m2x[移除硬编码的加密密钥]
* https://github.com/neuvector/neuvector/security/advisories/GHSA-qqj3-g7mx-5p4w[强制对遥测服务器请求进行 TLS 验证]
* https://github.com/neuvector/neuvector/security/advisories/GHSA-c8g6-qrwh-m3vp[检查 Enforcer 的 Consul 端口]

有关更多信息，请参阅 xref:cve.adoc[安全公告和 CVE] 文档。
...
* https://github.com/neuvector/neuvector/security/advisories/GHSA-8pxw-9c75-6w56[管理员账户具有不安全的默认密码]
* https://github.com/neuvector/neuvector/security/advisories/GHSA-8ff6-pc43-jwv3[不安全的密码管理易受彩虹攻击]
* https://github.com/neuvector/neuvector/security/advisories/GHSA-w54x-xfxg-4gxq[带有敏感参数的进程导致泄露]
...
[NOTE]
====
NeuVector Manager 和注册表适配器服务的默认类型已更改为 ClusterIP。 如果更喜欢使用 NodePort，用户仍然可以通过覆盖 `manager.svc.type` 和 `cve.adapter.svc.type` 的设置来实现。
====
...
- 在 Helm 图表中启用 `internal.autoGenerateCert` 和 `internal.autoRotateCert` 标志（从 5.4.2 版本开始，这些将默认启用）。或者，下面链接了 YAML 方法。

===== 使用 YAML：

- 使用现有方法提供一个 xref:internal.adoc#_replacing_certificates_used_in_internal_communications_of_suse_security[内部证书]。
...
关于 xref:internal.adoc[内部证书] 的更多信息。
...
* **NVSHAS-8700**:Rancher AD 用户有时无法登录到 {product-name}。
...
* 允许用户在使用 Helm 图表安装 {product-name} 时为 `updater-cron-job` 设置资源。
...
* 从之前的 {product-name} 版本升级到 v5.3.1 后，现有的 NvClusterSecurityRule 自定义资源可能会被意外删除。注意：为了防止升级问题，5.3.1 版本已从 Docker Hub 中移除。
...
==== {product-name} UI 扩展 v.Rancher 1.0 版，2024 年 3 月

* 安装 {product-name} 后，通过 Rancher xref:rancher.adoc#_suse_security_ui_extension_for_rancher[启用/安装] 的 {product-name} UI 扩展将显示集群仪表板，其中包含指向完整 {product-name} 集群 SSO 的链接。注意：该扩展可能显示为第三方，这将在未来的版本中修复。此外，安装后，Rancher 2.7.x 用户可能会在列表中看到两个 {product-name} UI 扩展图标（错误）。一个图标将显示为卸载（表示已安装），另一个应显示为安装。这可以保持不变，即如果扩展已安装，则不要再次安装。
...
* 添加对 ARM64 平台的支持。从基于 ARM 的平台拉取将自动拉取适当的 ARM64 {product-name} 镜像。
* 通过代理支持 webhook。
...
* 在合规性报告中为必须由用户手动运行的CIS基准添加“手动”状态（不是由 {product-name} 运行）。
* 改善漏洞页面的UI加载/性能。
* 统一浏览器会话登录。这样，浏览器中的所有标签共享相同的登录会话，从现有会话打开新标签时不需要输入凭据，当一个标签注销时，所有标签都将注销。
* 增强控制台（UI）的安全性：1）添加强制安全头（X-Content-Type-Options nosniff；X-XSS-Protection 1；模式=阻止；X-Frame-Options SAMEORIGIN；Cache-Control 私有，无缓存，无存储，必须重新验证
HTTP严格传输安全最大年龄=15724800，2）添加CSP头（例如，设置一个 '`default-src`' 指令），3）删除服务器名称泄露。
...
* 示例 Kubernetes 部署文件已从 {product-name} 文档中移除。请参考链接以获取示例。

==== 可能需要手动部署的显著更改（所有更改已在最新的 5.3.x Helm 图表中体现）。
 ...
 ** JWT 签名的证书由控制器自动创建，有效期为 90 天，并会自动轮换。
 ** 管理、REST API 和注册适配器证书的自动生成需要使用 {product-name} Helm 版本 2.6.3 或更高版本的基于 Helm 的安装。
 ** 如果在部署期间未被替换，内置证书仍然用于基于 yaml 的部署；但是，建议替换这些证书（见下一行）。
 ** 手动 xref:replacecert.adoc[替换证书] 仍然支持，并建议用于以前的版本或基于 yaml 的部署。请参阅 {product-name} GitHub 安全公告 https://github.com/neuvector/neuvector/security/advisories/GHSA-622h-h2p8-743x[这里]，获取详细描述。
 ** 用户提供的证书仍然像以前一样支持 Helm 和基于 yaml 的部署。
...
* 通过专用功能改善名称空间的修剪。
* {product-name} 名称空间用户无法查看资产--名称空间
...
* 在初始 {product-name} 部署中，不要使用默认密码创建默认管理员（CSP 适配器）以进行 AWS 计费，要求用户使用密钥创建管理员用户名和密码
* 修复 .json 文件，该文件增大了大小并导致 Kubernetes 节点崩溃
* 改进 SQL 注入检测逻辑
* 在安装 {product-name} 核心图表之前，首先安装 helm crd 图表时，服务帐户缺失
* 图像扫描 I.4.1 合规性结果不正确。
...
* 支持 {product-name} API 访问的词元。请查看设置 -> 用户，API 密钥 ... 以创建新的 API 密钥。密钥可以设置为默认或自定义角色。
* 支持 AWS Marketplace PAYG 计费的 {product-name} 月度支持订阅。用户可以通过 SUSE 支持订阅 {product-name}，按月计费到他们的 AWS 账户，基于上个月的平均节点使用量。有关更多信息，请参阅 xref:awsmarketplace.adoc[AWS marketplace]。
* 支持准入控制的图像签名。用户可以要求 {product-name} 验证图像是否由特定方签名，才能部署到生产环境中，通过与 Sigstore/Cosign 的集成。请查看资产 -> Sigstore 验证器以创建新的签名资产。然后可以使用图像签名和/或图像 Sigstore 验证器的标准创建规则。
...
* 支持通过 https://github.com/goharbor/pluggable-scanner-spec[可插拔扫描器] 接口从 Harbor 注册表调用 {product-name} 扫描器。这需要配置与控制器的连接（公开的 API）。Harbor 适配器调用控制器端点以触发扫描，该扫描可以在推送时自动进行。可以使用询问服务进行定期扫描。来自联邦主控制器的扫描结果会传播到远程集群。 注意：HTTPS 适配器端点存在错误：请忽略测试连接错误，虽然显示错误，但实际上仍能正常工作（跳过证书验证）。
* 可搜索的 SaaS 服务用于 CVE 查询。搜索最新的 {product-name} CVE 数据库，以查看特定 CVE 是否存在于数据库中。该服务可供 {product-name} Prime（付费支持订阅）客户使用。通过您的 SCC 门户联系支持以获取访问权限。
* 允许用户禁用网络保护，但保持 WAF/DLP 功能正常。在设置 -> 配置中配置网络策略启用。
* 根据每个 {product-name} 组件的要求使用权限较低的服务帐户。引入了一个变量 "`leastPrivilege`"。默认值为 false。注意：在 5.2.0 之前的版本中使用当前的 helm chart 和此变量将无法正常工作。
* 绑定到非默认服务帐户以满足 CIS 1.5 5.1.5 的建议。
* 允许管理员在设置 -> 用户、API 密钥和角色中配置用户默认会话超时。
* 可自定义的登录横幅和可自定义的 UI 头部文本，适用于受监管和政府部署。配置要求可以在xref:customui.adoc[这里]找到。
* SYSLOG 支持 TLS 加密传输。在设置 -> 中选择 TCP/TLS 配置 SYSLOG。
* 从 Rancher Manager 启用 {product-name} 监控器 Helm 图表的部署。
...
* 为响应规则添加并显示评论字段。
* 通过环境变量指定{product-name}认为的系统容器。例如，对于 Rancher 和默认名称空间：NV_SYSTEM_GROUPS=*cattle-system;default
...
* 从 {product-name} 4.x 版本升级时，错误的 pod 组成员导致意外的策略违规
...
* 构建工具现在可用于 {product-name}/Open Zero Trust (OZT) 项目，地址为 https://github.com/openzerotrust/openzerotrust.io.
* {product-name} 现在列出了控制器、管理器和执法者每个版本的版本 ID 和 SHA256 摘要，地址为 https://github.com/neuvector/manifests/tree/main/versions.
* 匿名遥测数据（节点数、组、规则）现在在部署时报告给 Rancher 云服务，以帮助项目团队了解使用行为。可以在 UI 中或通过 configMap (No_Telemetry_Report) 或 REST API 禁用此功能（选择退出）。
* （附录 2023 年 1 月）。支持基于 ServiceEntry 的网络策略，与 Istio 配合使用。在版本 5.1.0 中，为通过 Istio 声明的 ServiceEntry 目标的 Pod 添加了出站网络策略强制执行功能。通常，ServiceEntry 定义了如何将通过 DNS 名称引用的外部服务解析为目标 IP。在 v5.1 之前，{product-name} 无法检测和强制执行对 ServiceEntry 的连接规则，因此所有连接都被归类为外部。在 5.1 中，可以对特定的 ServiceEntry 目标强制执行规则。重要说明：如果您正在使用基于 Istio 的部署升级到 v5.1，则必须创建新规则以允许这些连接并避免违规警报。升级后，如果不存在允许规则，将报告隐式违规的新可见流量。新的流量规则可以在发现模式下学习和自动创建。要允许此流量，您可以将组置于发现模式，或创建一个包含地址（或 DNS 名称）的自定义组，并为该目标添加新的网络规则以允许流量。注意：在 5.1.0 中，拒绝违规报告的目标存在一个错误，该目标并不代表正确的目的地。 该错误报告服务器名称和客户端名称相同。 此问题将在即将发布的补丁中解决。
...
* 扫描器：在重建镜像库时实现清除属性。
* 验证 {product-name} 部署并支持启用了 SELinux 的主机。请参见下面有关临时修补程序的详细信息，直到 Helm 图表更新。
...
* 当SSO使用Rancher默认管理员登录到{product-name}的主集群时，{product-name}的登录角色是admin，而不是fedAdmin。
* 修复多个goroutine崩溃。
* 主机IP未与节点关联的隐式违规。
...
{product-name} 不需要任何额外设置即可在启用 SELinux 的集群中部署和运行。在基于 RHEL 8.5 的启用 SELinux 的 RKE2 加固集群上测试部署了 {product-name}。如果启用 PSP 并对管理器和扫描器部署进行补丁处理，则 {product-name} 能成功部署。下一个图表版本应修复以下问题。
...
* 组模式的自动推广功能。根据经过的时间和标准提升组的保护模式。不适用于由CRD创建的组。此功能允许新应用程序在发现模式下运行一段时间，学习行为并为网络和进程{product-name}创建白名单规则，然后自动转到监控模式，再到保护模式。从发现到监控的判定标准：在组中至少一个活动 pod 的所有网络和进程活动的学习耗时。从监控到保护的判定标准：在为该组设置的时间范围内没有安全事件（网络、进程等）。
* 支持Rancher 2.6.5应用程序和市场图表。部署到 cattle-neuvector-system 名称空间，并启用从 Rancher 到 {product-name} 的 SSO。注意：必须完全去除来自 Rancher 的先前部署（例如，合作伙伴目录图表，版本 1.9.x 及更早版本），才能更新到新图表。
...
* 通过 Rancher Manager 启动 {product-name} 控制台的 Rancher SSO 集成。此功能仅在通过 Rancher 部署 {product-name} 容器时可用。此部署从镜像 Rancher 仓库中拉取（例如 rancher/mirrored-neuvector-controller:5.0.0），并部署到 cattle-neuvector-system 命名空间。注意：需要更新的 Rancher 版本 2.6.5（2022 年 5 月）或更高版本，并且目前仅支持管理员和集群拥有者角色。
* 支持在 RKE2 上进行部署。
...
=== 从 {product-name} 4.x 升级到 5.x（在 5.2.x 之前）。

[NOTE]
====
以下说明适用于升级到 5.0.x 和 5.1.x。对于 5.2.x，服务帐户和绑定已更改，应 xref:kubernetes.adoc#_deploy_suse_security[审查] 以规划升级。
====


对于 Helm 用户，更新到 {product-name} Helm 图表 2.0.0 或更高版本。如果在OpenShift上更新 Operator 或Helm安装，请参阅下面的说明。
...
. 更新从 Docker hub (docker.io) 拉取 {product-name} 镜像的名称和路径，例如。
...
可选择性地，在 Helm 图表、部署 yaml、配置映射、脚本等中去除对 {product-name} 许可证和注册表机密的任何引用，因为这些不再需要拉取图像或开始使用 {product-name}。

*关于 SCC 和通过 Operator/Helm 升级的说明*

特权 SCC 在新部署中由操作员版本 1.3.4 及以上添加到部署 yaml 中指定的服务帐户。在将 {product-name} Operator 从先前版本升级到 1.3.4 或将 Helm 升级到 2.0.0 的情况下，请在升级之前删除特权 SCC。
...
* 功能完整，包括组模式的自动推广。根据经过的时间和标准提升组的保护模式。不适用于 CRD 创建的组。此功能允许新应用程序在发现模式下运行一段时间，学习行为并为网络和进程{product-name}创建白名单规则，然后自动转到监控模式，再到保护模式。从发现到监控的判定标准：在组中至少一个活动 pod 的所有网络和进程活动的学习耗时。从监控到保护的判定标准：在为该组设置的时间范围内没有安全事件（网络、进程等）。
* 支持 Rancher 2.6.5 应用程序和市场图表。部署到 cattle-neuvector-system 名称空间，并启用从 Rancher 到 {product-name} 的 SSO。注意：必须完全去除来自Rancher的先前部署（例如，合作伙伴目录图表，版本1.9.x及更早版本），才能更新到新图表。
...
* WAF、DLP和准入控制的CRD。注意：需要额外的集群角色绑定/权限。请参阅Kubernetes和OpenShift部署部分。通过CRD支持的准入控制的CRD导入/导出和版本控制。
* 通过 Rancher Manager 启动 {product-name} 控制台的 Rancher SSO 集成。此功能仅在通过 Rancher 部署 {product-name} 容器时可用。注意：需要更新的Rancher版本（日期/版本待定）。
* 支持在 RKE2 上进行部署。
* 通过代理支持集群的联邦（多集群管理）。
* 监控所需的 rbac 的 clusterrole/bindings，并在事件和 UI 中警报如果有任何缺失。
* 支持准入控制规则中的资源限制标准。
...
* 首次发布不受支持的 '技术预览' 版本的 {product-name} 5.0 开源版本。
