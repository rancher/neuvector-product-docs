...
{product-name}可以配置以扫描在GitLab构建管道中触发的漏洞。这里有一个可以配置和使用的 https://gitlab.com/neuvector/gitlab-plugin[GitLab插件]。请按照GitLab网站上的说明使用该插件。

扫描还可以通过配置下面提供的脚本来使用{product-name} REST API以访问控制器。

此外，请确保部署并配置了一个{product-name}扫描器容器，以连接到Allinone或控制器。在4.0及更高版本中，neuvector/scanner容器必须与allinone或控制器分开部署。

=== 使用REST API在GitLab构建期间扫描

使用以下脚本，配置您的{product-name}登录凭据以触发漏洞扫描。
