...
== Escanear vulnerabilidades durante el pipeline de compilación de Gitlab

{product-name} se puede configurar para escanear vulnerabilidades desencadenadas en el pipeline de compilación de Gitlab. Hay aquí un https://gitlab.com/neuvector/gitlab-plugin[complemento de Gitlab] que se puede configurar y utilizar. Por favor, sigue las instrucciones en el sitio de Gitlab para utilizar el complemento.

El escaneo también puede utilizar la API REST de {product-name} configurando el script proporcionado a continuación para acceder al controlador.

Además, asegúrate de que hay un {product-name} contenedor de escáner desplegado y configurado para conectarse al Allinone o al Controlador. En la versión 4.0 y posteriores, el contenedor neuvector/scanner debe desplegarse por separado del allinone o del controlador.

=== Escanear durante la compilación de Gitlab utilizando la API REST

Utiliza el siguiente script, configurado para tus credenciales de inicio de sesión de {product-name} para activar los escaneos de vulnerabilidades.
