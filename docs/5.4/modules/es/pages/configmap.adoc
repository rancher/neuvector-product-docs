...
{product-name} admite la configuración automatizada utilizando la función ConfigMap de Kubernetes. Esto permite el despliegue de {product-name} contenedores con las configuraciones, integraciones y otros ajustes apropiados de manera automatizada.
...
=== Ejemplo completo de ConfigMap {product-name} (initcfg.yaml)

El último ConfigMap se puede encontrar en el siguiente https://raw.githubusercontent.com/neuvector/manifests/main/kubernetes/5.4.0/initcfg.yaml[initcfg.yaml].
...
Ten en cuenta que el secreto se menciona en los archivos yaml de despliegue estándar de Kubernetes y OpenShift xref:kubernetes.adoc#_deploy_suse_security[deployment yaml files] bajo Volúmenes.

[#_configmaps_and_persistent_storage]
== ConfigMaps y almacenamiento persistente

Tanto los ConfigMaps como la copia de seguridad del xref:production.adoc#_backups_and_persistent_data[almacenamiento persistente] solo se leen cuando se despliega un nuevo {product-name} clúster, o cuando el clúster falla y se reinicia. No se utilizan durante las actualizaciones progresivas.
