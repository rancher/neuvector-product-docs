= Groups
:description: Manage security groups to view and customize rules, switch between Discover, Monitor, and Protect modes, and apply policies to containers and hosts
...
[#_auto_deletion_of_unused_groups]
=== Auto Deletion of Unused Groups

Learned groups (not reserved or custom groups) can be automatically deleted by {product-name} if there are no members (containers) in the group. The time period for this is configurable in Settings _> Configuration.

[#_host_protection_the_nodes_group]
...
[#_virtual_host_vh_based_network_policy]
