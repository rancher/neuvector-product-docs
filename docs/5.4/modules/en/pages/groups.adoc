...
The Groups screen also displays a 'Scorable' icon in the upper right, and a learned group can be selected and the Scorable checkbox enabled or disabled. This controls which containers are used to calculate the Security Risk Score in the Dashboard. See xref:improve-score.adoc#_improving-the-security-risk-score[Improve Security Risk Score] for more details.
...
[#_auto-deletion-of-unused-groups]
...
[#_host-protection-the-nodes-group]
...
[#_virtual-host-vh-based-network-policy]
