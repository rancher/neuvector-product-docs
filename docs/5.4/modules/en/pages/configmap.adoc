...
:description: Deploy SUSE® Security containers using a Kubernetes ConfigMap to automate configuration with settings, integrations, and reload options to manage updates
...
Note that the secret is referred to in the standard Kubernetes and OpenShift Controller xref:kubernetes.adoc#_deploy_product[deployment yaml files] under Volumes.
