...
NOTE: The environment variable NV_PLATFORM_INFO=platform=Docker is used to notify {product-name} that the platform is Docker/Swarm, even though there may be unused Kubernetes containers detected by {product-name} on a Docker EE deployment. Also to be able to see these in Network Activity _> View _> Show System, add the environment variable for the Enforcer NV_SYSTEM_GROUPS.
...
allinone:
    ...
    environment:
        ...
        - STORE_PASSPHRASE_FILE=/run/secrets/nv_store_passphrase
    ports:
        - 18300:18300
        - 18301:18301
        - 18400:18400
    ...
    volumes:
        ...
        - /path/on/host/store_passphrase:/run/secrets/nv_store_passphrase:ro
----
...
*STORE_PASSPHRASE_FILE* (required as of NeuVector v5.6.2+) points to a file containing the passphrase used for state encryption. Non-Kubernetes deployments have no automatic equivalent of the Kubernetes-managed encryption secret, so this must be provided explicitly, or the controller will fail to start. Generate one with:

[,shell]
----
openssl rand -base64 48 > /path/on/host/store_passphrase
chmod 600 /path/on/host/store_passphrase
----

Back this file up separately from /var/neuvector — it is a master key for encrypted state, and losing it may prevent decrypting persisted configuration.
...
[#_deploy_the_product_scanner_container]
