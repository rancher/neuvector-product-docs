...
:description: Manage enterprise multi-cluster deployments using the SUSE Security console by promoting primary clusters and joining remote clusters with tokens,
...
[#_configuring_the_primary_and_remote_clusters]
...
[#_federating_results_from_ci_cd_scanners_optional]
=== Federating Results from CI/CD Scanners (Optional)

[NOTE]
====
The federation of scan results to managed clusters is now an optional setting. By default, this setting is turned `OFF`.
====
