...
[NOTE]
====
It is not recommended to deploy (scale) more than one manager behind a load balancer due to potential session state issues. If you plan to use a PersistentVolume claim to store the backup of {product-name} config files, please see the general Backup/Persistent Data section in the xref:production.adoc#_backups-and-persistent-data[Deploying {product-name}] overview.
====
...
[#_using-node-labels-for-manager-and-controller-nodes]
...
[#_sample-kubernetes-rolling-update]
...
[#_expose-rest-api-in-kubernetes]
...
[#_kubernetes-deployment-in-non-privileged-mode]
