= DLP & WAF Sensors
:description: Configure DLP and WAF sensors to inspect network traffic using regular expressions and apply the regular expressions to groups for security filtering
...
[#_data_loss_prevention_dlp_and_web_application_firewall_waf]
...
[#_configuring_dlp_and_waf_sensors]
...
[#_configuring_federated_dlp_and_waf_sensors]
...
[#_applying_dlp_waf_sensors_to_container_groups]
...
[#_dlp_waf_actions_in_discover_monitor_protect_modes]
...
[#_testing_the_log4j_waf_detection]
...
[#_managing_waf_rules_using_import_export_or_crds]
...
[#_dlp_waf_response_rules]
== DLP/WAF Response Rules

Response rules based on DLP/WAF security events can be created in Policy _>Response Rules. Start type DLP or WAF and the dropdown will list all sensors and patterns available to create rules.
