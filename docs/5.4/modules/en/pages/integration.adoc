...
:description: Find integration options for SUSE® Security, including REST API, CLI, Syslog, RBAC, SAML, LDAP, webhooks, ecosystem partners like Sonatype, IBM Cloud
...
{product-name} provides multiple integration options, including a REST API, CLI, Syslog, RBAC, SAML, LDAP, and webhooks. For examples of automating tasks by using the REST API, see the xref:automation.adoc[*Automation*] section.
...
Enable this option if you use Red Hat OpenShift role_based access control (RBAC) and want {product-name} to read and enforce those permissions automatically.
