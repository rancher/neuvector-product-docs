...
:description: Deploy parallel and standalone scanners to increase performance and scalability by scanning images in registries using Kubernetes, and to scale up or down
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /06.scanning/06.scanners/06.scanners.md
:page-opendocs-slug:  /scanning/scanners

[#_increase_scanner_scalability_with_multiple_scanners]
...
=== Auto_scaling of Scanner Pods

Scanner pods can be configured to auto_scale based on certain criteria. This will ensure that scanning jobs are handled quickly and efficiently, especially if there are thousands of images to be scanned or re_scanned. There are three possible settings: delayed, immediate,  and disabled. When images are queued for scanning by the controller, it keeps a 'task count' of the queue size.
...
Scanner auto_scaling is configured in Settings _> Configuration. The minimumScannerPods setting sets the minimum scanner pods running at any time, while the maxScannerPods sets the maximum number of pods that the auto_scaling strategy can scale up to. NOTE: Setting a minimum value will not adjust the original scanner deployment replicaset value. The minimum value will be applied during the first scale up/down event.
...
[#_standalone_scanner_for_local_scanning]
...
[#_host_scanning_in_standalone_mode]
...
[#_manual_deployment_of_multiple_scanners_on_kubernetes]
