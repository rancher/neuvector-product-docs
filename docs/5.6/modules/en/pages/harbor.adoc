...
:description: Configure the Harbor Pluggable Scanner Module to scan Harbor registries using the registry adapter and controller
:revdate: 2024-09-27
:page-revdate: {revdate}
:page-opendocs-origin: /06.scanning/02.registry/01.harbor/01.harbor.md
:page-opendocs-slug:  /scanning/registry/harbor

[#_scanning_harbor_registries_using_the_pluggable_scanner]
...
[#_deploying_the_product_registry_adapter]
...
The adapter endpoint must be entered, and the adapter connects to the controller, which is typically exposed as a service externally so the adapter can connect to it. In addition, authentication credentials set through cve.adapter.harbor.secretName could be entered.
If you set cve.adapter.harbor.secretName in your Helm values:
It sets the HARBOR_BASIC_AUTH_USERNAME and HARBOR_BASIC_AUTH_PASSWORD environment variables inside the adapter pod.
The adapter will now act as a gatekeeper. When Harbor sends a request, the adapter will check the credentials against this secret first.

[#_scanning_images_from_a_harbor_registry]
