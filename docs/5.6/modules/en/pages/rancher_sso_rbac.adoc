...
:description: Create custom roles in Rancher RBAC for SUSE Security with specific permissions based on user groups
:revdate: 2026-02-05
...
[#_supported_custom_product_role_mapping_in_rancher_sso]
...
[#_resource_display_and_logical_name_mapping]
...
[#_definitions_and_expectations_with_global_cluster_and_project_namespace_roles]
== Definitions and expectations with Global, Cluster, and Project/Namespace roles

* Cluster resource with `*` verb on a Rancher Cluster role:
...
You should use the Cluster role to divide access scenarios based on the RBAC verbs and resources allowed by {product-name}. The Global role should be used strictly if you want to propagate to more clusters. Users must create Cluster roles to propagate the Global role to all downstream clusters.
====
