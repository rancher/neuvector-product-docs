...
[#_creating-custom-scripts-for-compliance-checks]
...
[NOTE]
====
The ability to create custom scripts is disabled by default to protect against misuse. This can be enabled be setting the CUSTOM_CHECK_CONTROL xref:details.adoc#_environment-variables[environment variable] in the Controller and Enforcer. Values are "disable" (default, not allowed), "strict" (admin role only), or "loose" (admin, compliance, and runtime-policy roles).
====
...
[#_sample-script-to-check-if-container-has-root-account-with-no-password]
...
[#_sample-script-to-check-dirty-cow-file-in-the-container]
...
[#_creating-a-custom-check-script]
...
[#_creating-a-response-rule-for-compliance-report]
