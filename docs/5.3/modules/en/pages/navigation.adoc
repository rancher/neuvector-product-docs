...
:description: Access the console using the default admin credentials and navigate menus to view security risk scores, settings, and network activity details
:revdate: 2024-09-27
...
[#_x_forwarded_for_behavior_details]
===== X_FORWARDED_FOR Behavior Details
...
In order to preserve the original source IP address, the user needs to add the following line to the exposed services, in the 'spec' section of the external facing load balancer or ingress controller. (Ref: https://kubernetes.io/docs/tutorials/services/source_ip/)
...
[#_settings_ldap_ad_saml_and_openid_connect]
...
[#_icon_descriptions_in_legend_network_activity]
...
[#_group_container_service_mesh_in_discovery]
...
[#_group_container_service_mesh_being_monitored]
...
[#_group_container_service_mesh_being_protected]
