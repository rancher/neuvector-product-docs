...
To update the Scanner in order to get the latest CVE database updates from NeuVector, create a cron job to stop and restart the scanner, pulling the latest. See xref:updating.adoc#_docker_native_updates[this section] for details.
