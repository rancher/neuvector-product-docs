...
:description: Deploy the SUSE® Security containers, including controller, manager, enforcer, scanner, and updater, with proper placement and image versions
:revdate: 2024-09-27
...
[#_best_practices_tips_qa_for_deploying_and_managing_product]
...
Automated deployment using Helm can be found at https://github.com/neuvector/neuvector_helm.
...
== Handing Host Updates or Auto_Scaling Nodes with a Pod Disruption Budget

Maintenance or scaling activities can affect the controllers on nodes. Public cloud providers support the ability to auto_scale nodes, which can dynamically evict pods including the {product-name} controllers. To prevent disruptions to the controllers, a {product-name} pod disruption budget can be created.
...
== Multi_site, Multi_Cluster Architecture
